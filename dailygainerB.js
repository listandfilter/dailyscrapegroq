...
export async function scrapeStockDataB() {
  const browser = await puppeteer.launch({ headless: true ,
    args: ["--no-sandbox", "--disable-setuid-sandbox"],
  });
  const page = await browser.newPage();

  await page.goto('https://money.rediff.com/gainers/bse/daily/groupb', { waitUntil: 'networkidle2' });
  ...
  // Save to file
  const filePath = path.join('./', 'dailygainerB.csv');
