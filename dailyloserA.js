...
export async function scrapeStockDataloser() {
  const browser = await puppeteer.launch({ headless: true ,
    args: ["--no-sandbox", "--disable-setuid-sandbox"],
  });
  const page = await browser.newPage();

  await page.goto('https://money.rediff.com/losers/bse/daily/groupa', { waitUntil: 'networkidle2' });
  ...
  // Save to file
  const filePath = path.join('./', 'dailyloserA.csv');
