...
function extractStockNameFromCSV(filePath = './combined_gain.csv') {
...
async function getCompanyFeed(stockName) {
  ...
  try {
    ...
    const [rows] = await connection.query(
      `SELECT content
      FROM wp_stockedge_dailygain_gain_investigator
      WHERE stockName = ?
...
async function sendToWordPress( stockName,changePercent, reasons, tag = 'dailygainer') {
  try {
...
export async function rundailygain() {
  
  const companies = extractStockNameFromCSV('./combined_gain.csv');
